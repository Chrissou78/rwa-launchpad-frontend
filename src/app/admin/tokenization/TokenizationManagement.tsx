...
export default function TokenizationManagement({ onRefresh }: TokenizationManagementProps) {
  ...
  const handleUpdateApplication = async () => {
    ...
    try {
      ...
      console.log('Current status:', selectedApp.status, '→ New:', newStatus);
      console.log('On-chain approved:', onChainStatus.isApproved);

      // On-chain approval when: approving AND not yet approved on-chain
      const needsOnChainApproval = newStatus === 'approved' && !onChainStatus.isApproved;
      
      // On-chain revocation when: rejecting (regardless of current on-chain status)
      const needsOnChainRevocation = newStatus === 'rejected';

      console.log('needsOnChainApproval:', needsOnChainApproval);
      console.log('needsOnChainRevocation:', needsOnChainRevocation);

      // Approve deployer on-chain
      if (needsOnChainApproval) {
        if (!contracts?.RWALaunchpadFactory) {
          setError('Factory contract not found. Check network connection.');
          setUpdating(false);
          return;
        }

        console.log('Calling approveDeployerOnChain for:', selectedApp.user_address);
        try {
          await approveDeployerOnChain(selectedApp.user_address);
          console.log('On-chain approval successful!');
        } catch (contractErr: any) {
          console.error('On-chain approval failed:', contractErr);
          setError(`On-chain approval failed: ${contractErr.message}`);
          setUpdating(false);
          return;
        }
      }

      // Revoke deployer on-chain
      if (needsOnChainRevocation) {
        if (!contracts?.RWALaunchpadFactory) {
          setError('Factory contract not found. Check network connection.');
          setUpdating(false);
          return;
        ...
        console.log('Calling revokeDeployerOnChain for:', selectedApp.user_address);
        try {
          await revokeDeployerOnChain(selectedApp.user_address);
          console.log('On-chain revocation successful!');
        } catch (contractErr: any) {
          console.error('On-chain revocation failed:', contractErr);
          setError(`On-chain revocation failed: ${contractErr.message}`);
          setUpdating(false);
          return;
        }
      }
                ...
                <div className="space-y-4">
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      >
                        <option value="pending">Pending Review</option>
                        <option value="approved">Approved (Ready to Deploy)</option>
                        <option value="completed">Completed (Deployed)</option>
                        <option value="rejected">Rejected</option>
                        <option value="cancelled">Cancelled</option>
                      </select>
                      {newStatus === 'approved' && !onChainStatus.isApproved && (
                        <p className="text-xs text-green-400 mt-1">
                          ✓ Will approve deployer on-chain
                        </p>
                      )}
                      {newStatus === 'rejected' && (
                        <p className="text-xs text-red-400 mt-1">
                          ✗ Will revoke on-chain approval. User can modify and resubmit.
                        </p>
                      )}
                    </div>
