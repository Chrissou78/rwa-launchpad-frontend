...
import { ZERO_ADDRESS, getChainInfo, SupportedChainId } from '@/config/contracts';
...
export default function ExchangeClient() {
  ...
  console.log('[ExchangeClient] deployedChains:', deployedChains, 'type:', typeof deployedChains, 'isArray:', Array.isArray(deployedChains));
  ...
  // Get coin symbol for deposit
  const getDepositCoin = useCallback(() => {
    if (depositToken === 'native') {
      // Handle Polygon rebrand from MATIC to POL
  ...
  // Fetch MEXC deposit address
  const fetchDepositAddress = useCallback(async () => {
    // Don't fetch for testnets
    if (isTestnet) {
      setDepositAddressError('Deposits only available on mainnet');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `/api/exchange/mexc/deposit-address?coin=${coin}&chainId=${chainId}`
      );
      ...
      if (response.ok && data.address) {
        ...
        setDepositAddressError(data.error || 'Failed to get deposit address');
      }
    } catch (error) {
      console.error('Error fetching deposit address:', error);
      setDepositAddressError('Failed to fetch deposit address');
    } finally {
      setIsLoadingDepositAddress(false);
    }
  }, [chainId, getDepositCoin, isTestnet]);

  // Fetch deposit address when modal opens or token changes
  useEffect(() => {
    if (showDepositModal) {
      fetchDepositAddress();
    }
  }, [showDepositModal, depositToken, fetchDepositAddress]);
  ...
  const fetchMexcOrderBook = useCallback(async (symbol: string) => {
    try {
      ...
      if (!response.ok) {
        console.error('[OrderBook] Failed:', response.status);
        return;
      }
      const data = await response.json();
      console.log('[OrderBook] Raw data:', data); // Debug log
      
      // Ensure asks and bids are arrays
      setMexcOrderBook({
  ...
  const loadWalletBalances = useCallback(async () => {
    ...
    try {
      ...
      const balances: Record<string, bigint> = {
        [nativeCurrency || 'NATIVE']: nativeBalance,
      };
  ...
  }, [publicClient, address, usdtAddress, usdcAddress, nativeCurrency]);
  ...
  const formatTokenAmount = (value: bigint | string | number): string => {
    let num: number;
  ...
  // Handle MEXC trade
  const handleMexcTrade = useCallback(async () => {
    if (!tradeAmount || parseFloat(tradeAmount) <= 0) return;
    if (orderType === 'limit' && (!limitPrice || parseFloat(limitPrice) <= 0)) return;

    const marketPrice = mexcTickers[selectedMexcPair]?.lastPrice
      ? parseFloat(mexcTickers[selectedMexcPair].lastPrice)
      : 0;
    const executionPrice = orderType === 'limit' ? parseFloat(limitPrice) : marketPrice;
    ...
    if (orderType === 'market') {
      alert(
        `${tradeType.toUpperCase()} Market Order Placed!\n\n` +
        `Pair: ${selectedMexcPair}\n` +
        `Amount: ${amount} ${selectedMexcPair.replace('USDT', '')}\n` +
        `Price: ${formatPrice(executionPrice)}\n` +
        `Total: $${total.toFixed(2)}\n\n` +
        `Note: This is a demo. Real trading requires MEXC API authentication.`
      );
    } else {
      const priceDiff = ((executionPrice - marketPrice) / marketPrice) * 100;
      alert(
        `${tradeType.toUpperCase()} Limit Order Placed!\n\n` +
        `Pair: ${selectedMexcPair}\n` +
        `Amount: ${amount} ${selectedMexcPair.replace('USDT', '')}\n` +
        `Limit Price: ${formatPrice(executionPrice)} (${priceDiff >= 0 ? '+' : ''}${priceDiff.toFixed(2)}% vs market)\n` +
        `Market Price: ${formatPrice(marketPrice)}\n` +
        `Total: $${total.toFixed(2)}\n\n` +
        `Order will execute when price reaches ${formatPrice(executionPrice)}.\n\n` +
        `Note: This is a demo. Real trading requires MEXC API authentication.`
      );
    }
  ...
  }, [tradeAmount, tradeType, orderType, limitPrice, selectedMexcPair, mexcTickers]);
  ...
  // Helper functions
  const formatPrice = (value: string | number, decimals?: number): string => {
    const num = typeof value === 'string' ? parseFloat(value) : value;
    if (isNaN(num)) return '$0.00';
    
    // Dynamic decimals: under $1 = 4 decimals, $1+ = 2 decimals
  ...
  // Format amount with quantity and USD value
  const formatAmount = (
    ...
    // Determine quantity decimals based on asset price
    let qtyDecimals = 2;
    if (price !== undefined) {
      if (price >= 2000) {
        qtyDecimals = 6;
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-8">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {safeDeployedChains.slice(0, 4).map((chain: any, index: number) => {
                ...
                const isTestnetChain = typeof chain === 'object' ? chain.testnet : false;
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-4 mb-8 border-b border-gray-800">
          <button
            ...
            className={`px-4 py-3 font-medium transition-colors ${activeTab === 'crypto'
            ...
            <div className="lg:col-span-2 bg-gray-900 rounded-xl p-4">
              ...
              {mexcTicker && (
                <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-800">
                  <div className="flex items-center gap-3">
                    <img 
                      src={getTokenIcon(selectedMexcPair.replace('USDT', ''))} 
                      alt={selectedMexcPair}
              ...
              {(() => {
                const currentPrice = mexcTickers[selectedMexcPair]?.lastPrice 
                  ? parseFloat(mexcTickers[selectedMexcPair].lastPrice) 
                  : undefined;
                  ...
                  <>
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {mexcOrderBook?.asks?.slice(0, 25).reverse().map((entry, i) => {
                        ...
                        {(!mexcOrderBook || !mexcOrderBook.asks?.length) && (
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {mexcOrderBook?.bids?.slice(0, 25).map((entry, i) => {
                          const { qty, usd } = formatAmount(entry[1], currentPrice);
                          return (
                            <div key={i} className="flex justify-between text-sm items-center">
                        ...
                        {(!mexcOrderBook || !mexcOrderBook.bids?.length) && (
            ...
            <div className="bg-gray-900 rounded-xl p-4">
              ...
              <div className="space-y-3">
                    ...
                    <div className="flex gap-2 mt-2">
                      {(() => {
                        ...
                        return (
                          <>
                            <button
                              onClick={() => setLimitPrice((currentPrice * 0.95).toFixed(currentPrice < 1 ? 4 : 2))}
                              className="flex-1 text-xs py-1 bg-gray-800 hover:bg-gray-700 rounded text-gray-400"
                            >
                              -5%
                            </button>
                            <button
                              onClick={() => setLimitPrice((currentPrice * 0.99).toFixed(currentPrice < 1 ? 4 : 2))}
                              className="flex-1 text-xs py-1 bg-gray-800 hover:bg-gray-700 rounded text-gray-400"
                            >
                              -1%
                            </button>
                            <button
                              onClick={() => setLimitPrice(currentPrice.toFixed(currentPrice < 1 ? 4 : 2))}
                              className="flex-1 text-xs py-1 bg-gray-800 hover:bg-gray-700 rounded text-gray-400"
                            >
                              Market
                            </button>
                            <button
                              onClick={() => setLimitPrice((currentPrice * 1.01).toFixed(currentPrice < 1 ? 4 : 2))}
                              className="flex-1 text-xs py-1 bg-gray-800 hover:bg-gray-700 rounded text-gray-400"
                            >
                              +1%
                            </button>
                            <button
                              onClick={() => setLimitPrice((currentPrice * 1.05).toFixed(currentPrice < 1 ? 4 : 2))}
                              className="flex-1 text-xs py-1 bg-gray-800 hover:bg-gray-700 rounded text-gray-400"
                            >
                              +5%
                            </button>
                    ...
                    <button
                      onClick={() => {
                        const price = orderType === 'limit' && limitPrice
                          ...
                          : parseFloat(mexcTickers[selectedMexcPair]?.lastPrice || '0');
                        if (price > 0) setTradeAmount((25 / price).toFixed(price >= 2000 ? 6 : price >= 500 ? 4 : 2));
                      }}
                      className="flex-1 text-xs py-1 bg-gray-800 hover:bg-gray-700 rounded text-gray-400"
                    >
                      $25
                    ...
                    <button
                      onClick={() => {
                        const price = orderType === 'limit' && limitPrice
                          ? parseFloat(limitPrice)
                          : parseFloat(mexcTickers[selectedMexcPair]?.lastPrice || '0');
                        if (price > 0) setTradeAmount((50 / price).toFixed(price >= 2000 ? 6 : price >= 500 ? 4 : 2));
                      }}
                      className="flex-1 text-xs py-1 bg-gray-800 hover:bg-gray-700 rounded text-gray-400"
                    >
                      $50
                    ...
                    <button
                      onClick={() => {
                        const price = orderType === 'limit' && limitPrice
                          ? parseFloat(limitPrice)
                          : parseFloat(mexcTickers[selectedMexcPair]?.lastPrice || '0');
                        if (price > 0) setTradeAmount((100 / price).toFixed(price >= 2000 ? 6 : price >= 500 ? 4 : 2));
                      }}
                      className="flex-1 text-xs py-1 bg-gray-800 hover:bg-gray-700 rounded text-gray-400"
                    >
                      $100
                    ...
                    <button
                      onClick={() => {
                        const price = orderType === 'limit' && limitPrice
                          ? parseFloat(limitPrice)
                          : parseFloat(mexcTickers[selectedMexcPair]?.lastPrice || '0');
                        if (price > 0) setTradeAmount((500 / price).toFixed(price >= 2000 ? 6 : price >= 500 ? 4 : 2));
                      }}
                      className="flex-1 text-xs py-1 bg-gray-800 hover:bg-gray-700 rounded text-gray-400"
                    >
                      $500
                ...
                {(() => {
                  ...
                  // Calculate price difference for limit orders
                  ...
                  return (
                    <div className="bg-gray-800/50 rounded-lg p-3 space-y-2">
                      ...
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-400">
                          Fee ({((MEXC_CONFIG.tradingFee + MEXC_CONFIG.platformFee) * 100).toFixed(1)}%)
                        </span>
                ...
                >
                  ...
                  {tradeType === 'buy' ? 'Buy' : 'Sell'} {selectedMexcPair.replace('USDT', '')}
                </button>
                ...
                <div className="space-y-2">
                  {securityTokens.map((token) => (
                    <button
                      ...
                      className={`w-full p-3 rounded-lg transition-colors text-left ${selectedSecurityToken?.address === token.address
                        ? 'bg-blue-500/20 border border-blue-500/30'
                        : 'bg-gray-800 hover:bg-gray-700'
                        }`}
                    >
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    <div className="bg-gray-800 rounded-lg p-3">
                      ...
                      <div className="font-medium">{formatTokenAmount(selectedSecurityToken.totalSupply)}</div>
                    </div>
                    <div className="bg-gray-800 rounded-lg p-3">
                      <div className="text-sm text-gray-400">Min Order</div>
                      <div className="font-medium">
                        {selectedSecurityToken.tradingPair
                          ? formatTokenAmount(selectedSecurityToken.tradingPair.minOrderSize)
                          : 'N/A'}
                      </div>
                    </div>
                    <div className="bg-gray-800 rounded-lg p-3">
                          ...
                          {selectedSecurityToken.orderBook.asks.length > 0 ? (
                            selectedSecurityToken.orderBook.asks.map((order) => (
                              <div key={order.orderId.toString()} className="flex justify-between text-sm bg-red-500/10 rounded px-2 py-1">
                                ...
                                <span className="text-gray-400">{formatTokenAmount(order.amount - order.filled)}</span>
                              </div>
                            ))
                          ) : (
                ...
                <>
                  ...
                  <div className="bg-gray-800 rounded-lg p-3 mb-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>{formatTokenAmount(getUserTokenBalance(selectedSecurityToken.address))}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 mb-4">
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg font-medium transition-colors ${orderSide === 'buy'
                    ...
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg font-medium transition-colors ${orderSide === 'sell'
        ...
        {/* Deposit Modal - MEXC Deposit Address */}
        {showDepositModal && (
              ...
              <div className="mb-4">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 py-2 rounded-lg font-medium transition-colors ${depositToken === 'native'
                  ...
                  {usdtAddress && (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg font-medium transition-colors ${depositToken === 'USDT'
                        ? 'bg-blue-500 text-white'
                        : 'bg-gray-800 text-gray-400'
                        }`}
                    >
                  ...
                  {usdcAddress && (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg font-medium transition-colors ${depositToken === 'USDC'
                        ? 'bg-blue-500 text-white'
                        : 'bg-gray-800 text-gray-400'
                        }`}
                    >
              ...
              <div className="mb-6">
                ...
                {isLoadingDepositAddress ? (
                  ...
                  <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4">
                    <p className="text-sm text-red-400">{depositAddressError}</p>
                    <button
